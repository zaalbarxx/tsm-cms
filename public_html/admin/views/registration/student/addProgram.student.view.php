  ...
  <?php if($eligiblePrograms){ ?>
		<?php foreach($eligiblePrograms as $program){ ?>
			<div class="smallItem">
				...
				<span class="buttons">
				<a href="index.php?com=registration&view=student&action=addProgram&student_id=<?php echo $studentInfo['student_id']; ?>&enrollInProgram=<?php echo $program['program_id']; ?>" class="addButton24" title="Add to <?php echo $studentInfo['first_name']; ?>"></a>
				</span>
			</div>  
		<?php } ?>
  <?php } else { ?>
