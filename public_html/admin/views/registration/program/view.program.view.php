...
<h1><?php echo $pageTitle; ?> - <a href="index.php?com=registration&view=programs&action=addEditProgram&program_id=<?php echo $programInfo['program_id']; ?>" class="editButton" title="Edit Program"></a></h2>
  ...
  <div class="courses">
  <h3>Available Courses - <a href="index.php?com=registration&view=programs&action=addCourse&program_id=<?php echo $program_id; ?>" class="addButton24 fb" style="bottom:-5px" title="Add a Course"></a></h3>
  <?php if(isset($programCourses)){
  	foreach($programCourses as $course){
  		?>
  		<div class="smallItem">
				<a href="index.php?com=registration&view=programs&action=viewCourse&course_id=<?php echo $course['course_id']; ?>&program_id=<?php echo $program_id; ?>" class="title"><?php echo $course['name']; ?></a>
				<span class="buttons"><a href="#" class="deleteButton deleteCourse" title="Remove Course" ref="<?php echo $course['course_id']; ?>"></a></span>
			</div>
  		<?php
  	} 
  } else {
  ...
  <?php if(isset($programFees)) { 
  	foreach($programFees as $fee){ ?>
			<div class="bigItem">
				...
				<span class="buttons"><a href="#" class="deleteButton deleteFee" title="Delete Fee" ref="<?php echo $fee['fee_id']; ?>"></a></span>
