...
<h1><?php echo $pageTitle; ?></h2>
  ...
  <h3>Enrollment Requirements - <a href="index.php?com=registration&view=courses&action=addRequirement&course_id=<?php echo $course_id; ?>&program_id=<?php echo $program_id; ?>" class="addButton24 fb" style="bottom:-5px" title="Add a Requirement"></a></h3>
  <?php
  ...
  <h3>Applicable Fees - <a href="index.php?com=registration&view=courses&action=addFee&course_id=<?php echo $course_id; ?>&program_id=<?php echo $program_id; ?>" class="addButton24 fb" style="bottom:-5px" title="Add a Fee"></a></h3>
  ...
  <?php if(isset($courseFees)) { 
  	foreach($courseFees as $fee){ ?>
			<div class="bigItem">
				...
				<b><span class="tooltip" title="This fee is applied only if applicant matches the condition(s) below.">Conditions:</span></b> - <a href="index.php?com=registration&view=courses&action=addCondition&course_id=<?php echo $course_id; ?>&fee_id=<?php echo $fee['fee_id'] ?>&program_id=<?php echo $program_id; ?>" class="fb">Add</a>
				<br />
				<?php 
				if($fee['conditions']){
					$i = 1;
