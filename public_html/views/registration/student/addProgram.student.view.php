  ...
  <?php if($eligiblePrograms){ ?>
		<?php foreach($eligiblePrograms as $program){ ?>
			<div class="bigItem" style="margin-left: auto; margin-right: auto;">
				...
				<span class="buttons">
				<a href="index.php?com=registration&action=addProgram&student_id=<?php echo $studentInfo['student_id']; ?>&enrollInProgram=<?php echo $program['program_id']; ?>" class="addButton24" title="Enroll <?php echo $studentInfo['first_name']; ?> in <?php echo $program['name']; ?>"></a>
				</span>
				<div class="itemDetails" style="display: block;">
					<div class="half">
						<span class="label">Registration Fee:</span> $<?php echo $reg->addFees($student->getFeesForProgramAndCourses($program['program_id'],2)); ?>
					</div>
					<div class="half">
						<span class="label">Tuition:</span> $<?php echo $reg->addFees($student->getFeesForProgramAndCourses($program['program_id'],1)); ?>
					</div>
				</div>
			</div>  
		<?php } ?>
